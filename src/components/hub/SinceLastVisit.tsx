...
const takeoverHeadingClass = "m-0 text-[32px] font-semibold leading-[1.1] tracking-[-0.035em]";
...
function TapFooter({ onContinue, final = false }: { onContinue: () => void; final?: boolean }) {
  return <div className="absolute inset-x-0 bottom-0 z-10 px-6 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-12 bg-gradient-to-t from-[#0a0a0c] via-[#0a0a0c]/95 to-transparent">
    <button className="mx-auto flex items-center gap-2 border-0 bg-transparent p-2 text-base font-semibold uppercase tracking-[0.12em] text-[#a0a0ab]" type="button" onClick={onContinue}>
...
function VideoMissed() {
  return <div className="pt-3">
    ...
    <div className="mt-4 overflow-hidden rounded-3xl border border-white/10">
      ...
      <div className="bg-[#17171a] px-3.5 py-3"><p className="m-0 text-xs font-bold uppercase tracking-[0.1em] text-gr-subtle">Featured video</p><h2 className="m-0 mt-1.5 text-[20px] font-bold leading-[1.2]">{lastVisit.video.title}</h2><p className="m-0 mt-1 text-sm text-gr-muted">{lastVisit.video.author} · {lastVisit.video.views}</p></div>
    </div>
    <h2 className="m-0 mt-4 text-[18px] font-bold uppercase tracking-[0.1em] text-gr-muted">Short takes</h2>
...
function StanceMissed() {
  return <div className="pt-3">
    <p className="m-0 text-sm font-bold uppercase tracking-[0.1em] text-gr-muted">Editorials</p>
...
function QuizMissed() {
  ...
  return <div className="pt-3">
    <p className="m-0 text-sm font-bold uppercase tracking-[0.1em] text-gr-muted">You missed this</p><h1 className={takeoverHeadingClass + " mt-2"}>Daily Quiz Waiting</h1>
...
function Recap({ onClose }: { onClose: () => void }) {
  return <div className="pt-3">
    <p className="m-0 text-sm font-bold uppercase tracking-[0.1em] text-gr-muted">Weekly wrap</p><h1 className={takeoverHeadingClass + " mt-2 uppercase"}>You’re all caught up</h1><p className="m-0 mt-4 text-lg leading-[1.4] text-gr-muted">Your personalized recap is complete. Here’s how you showed up this week.</p>
