...
function EditorialStanceCard({ stance }: { stance: HubEditorialStance }) {
  ...
  return (
    <article className="rounded-3xl border border-white/10 bg-[#16161a] p-4">
      ...
      <h3 className="m-0 mt-4 text-[17px] font-bold leading-[1.35] tracking-[-0.02em]">“{stance.quote}”</h3>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        {(["agree", "disagree"] as const).map((choice) => {
          ...
          return <button key={choice} className={"relative overflow-hidden rounded-2xl border px-3 py-3 text-sm font-semibold " + (chosen ? chosenStyle : "border-white/10 text-gr-subtle")} type="button" aria-pressed={chosen} onClick={() => setVote(choice)}>
      ...
      <div className="mt-4 border-t border-white/10 pt-4">
        ...
        <div className="mt-3 space-y-3">
          {comments.slice(0, 2).map((comment) => <div key={comment.id} className="flex gap-2.5"><span className="inline-flex size-6 shrink-0 items-center justify-center rounded-full bg-[#3b4454] text-[9px] font-bold">{comment.user.slice(1, 3).toUpperCase()}</span><div className="min-w-0"><p className="m-0 text-xs font-bold">{comment.user}</p><p className="m-0 mt-1 text-xs leading-[1.4] text-gr-muted">{comment.text}</p><span className="mt-1.5 inline-flex items-center gap-1 text-[11px] text-gr-muted"><ThumbsUp size={12} aria-hidden="true" />{comment.likes}</span></div></div>)}
        </div>
...
export function HubEditorialStances({ stances }: { stances: HubEditorialStance[] }) {
  return <section id="stances" className="scroll-mt-28 border-t border-[#38404e] py-8" aria-labelledby="stances-title">
    <p className="m-0 text-[11px] font-extrabold uppercase tracking-[0.14em] text-gr-muted">Editor stances</p>
    <h2 id="stances-title" className="m-0 mt-1 text-[24px] font-bold tracking-[-0.03em]">Bold viewpoints from our lead columnists</h2>
    <div className="mt-5 space-y-3">{stances.map((stance) => <EditorialStanceCard key={stance.id} stance={stance} />)}</div>
  </section>;
}
